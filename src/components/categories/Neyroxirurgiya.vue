...
Search(e){
  ...
  axios.post('https://itmed.herokuapp.com/api/Discharges/Search/neyroxirurgiya',
  {
number:this.number,
...
async clickCallback (pageNum = 1) {
  ...
  console.log(pageNum)
  this.load2 = true
  await axios.get(`https://itmed.herokuapp.com/api/Discharges/neyroxirurgiya/20/${pageNum}`, {
     headers: {
    'Authorization': `Bearer ${localStorage.getItem('jwt')}`
  }
  }).then(res => {
