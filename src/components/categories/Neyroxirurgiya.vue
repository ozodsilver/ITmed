<template>
   ...
   <input type="text" placeholder="Ism" class="p-1 form-control w-25"  v-model="searchText">
          ...
          <h1 class=" badge bg-grayish p-3 d-flex align-items-center">Neyroxirurgiya</h1>
  ...
  onMounted(async () => {
    let response = await axios.get("https://itmed.herokuapp.com/api/Discharges/neyroxirurgiya/20/1", {
       headers: {
  ...
  let Search = async(e)=>{
    ...
    let res = await axios.post('https://itmed.herokuapp.com/api/Discharges/Search/neyroxirurgiya ',
    {
  number:'',
  ...
  let response  =  await axios.delete(`https://itmed.herokuapp.com/api/neyroxirurgiya/${id}`, {
    headers: {
      'Authorization': `Bearer ${localStorage.getItem('jwt')}`,
      'Content-Type': 'application/json'
    }
