<template>
 ...
 <input type="text" placeholder=" Ism" class="p-1 p-sm-0 form-control w-50"  v-model="searchText">
        ...
        <h1 class=" badge bg-grayish py-2 mt-2   d-flex align-items-center" id="bulim"> Erta yosh bolalar poltologiyasi</h1>
...
Search(e){
  ...
  axios.post('https://itmed.herokuapp.com/api/Discharges/Search/erta',
  {
number:this.number,
...
async clickCallback (pageNum = 1) {
  ...
  console.log(pageNum)
  this.load2 = true
  await axios.get(`https://itmed.herokuapp.com/api/Discharges/erta/20/${pageNum}`, {
     headers: {
    'Authorization': `Bearer ${localStorage.getItem('jwt')}`
  }
  }).then(res => {
